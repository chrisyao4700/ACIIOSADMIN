//
//  OrderDiscountDetailViewController.m
//  ACI Hacienda
//
...
@interface OrderDiscountDetailViewController ()

@end

@implementation OrderDiscountDetailViewController{
...
-(IBAction)didClickRightItem:(id)sender{
    [self requestUpdateRecordForTable:@"aci_order_discount" forDataID:self.discountID forKey:@"status" forValue:@"1" withTag:11];
}

-(void) requestOrderDiscountDetailData:(NSString *) discountID{
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString * query = [NSString stringWithFormat:@"SELECT `aci_order_discount`.*, `aci_discount_category`.`discount_category` AS `category_str`, CONCAT (`aci_contact`.`first_name`, ' ', `aci_contact`.`last_name`) AS `creator_name` FROM `aci_order_discount` LEFT JOIN `aci_discount_category` ON `aci_discount_category`.`id` = `aci_order_discount`.`discount_category_id` LEFT JOIN `aci_users` ON `aci_users`.`id` = `aci_order_discount`.`cby` LEFT JOIN `aci_contact` ON `aci_contact`.`id` = `aci_users`.`contact_id` WHERE `aci_order_discount`.`id` = %@",discountID];
        NSDictionary * dict = @{
                                @"query":query
                                };
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self requestOrderDiscountDetailData:self.discountID];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [connector sendNormalRequestWithPack:dict andServiceCode:@"update_record" andCustomerTag:c_tag andWillStartBlock:^(NSInteger c_tag){
            ...
            if (c_tag == 11) {
                //ACTIVE || DELETED
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.delegate orderDiscountRequestUpdateOrderPrice];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            //ID
            cell.textLabel.text = [NSString stringWithFormat:@"DISCOUNT RULE # %@",self.discountID];
            cell.detailTextLabel.text = [NSString stringWithFormat:@"CREATED BY %@ -- %@",[discount_pack objectForKey:@"creator_name"], [CYFunctionSet convertDateToShortStr:[CYFunctionSet convertStringToDate:[discount_pack objectForKey:@"cdate"]]]];
        ...
        if (indexPath.row == 1) {
            ...
            cell.detailTextLabel.text = [discount_pack objectForKey:@"name"];
        }
        if (indexPath.row == 2) {
            cell.textLabel.text = @"NOTE";
            cell.detailTextLabel.text = [discount_pack objectForKey:@"note"];
        }
        if (indexPath.row == 3) {
            cell.textLabel.text = @"CATEGORY";
            cell.detailTextLabel.text = [discount_pack objectForKey:@"category_str"];
            cell.userInteractionEnabled = NO;
            cell.accessoryType= UITableViewCellAccessoryNone;
        }
        if (indexPath.row == 4) {
            cell.textLabel.text = [discount_pack objectForKey:@"category_str"];
            cell.userInteractionEnabled = NO;
            cell.accessoryType= UITableViewCellAccessoryNone;
            num_pool = [CYFunctionSet convertStingToNumber:[discount_pack objectForKey:@"discount_category_id"]];
            
            if (num_pool.integerValue == 0) {
                // PERCENTAGE
                
                cell.detailTextLabel.text = [NSString stringWithFormat:@"%@%% OFF",[discount_pack objectForKey:@"percentage"]];;
            }
            if (num_pool.integerValue == 1) {
                // CASH BACK
                cell.detailTextLabel.text = [discount_pack objectForKey:@"cashback"];
            }
        }
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row < 3) {
            ...
            [self performAlertViewTextModifyWithKey:key_pool forValue:[discount_pack objectForKey:key_pool] forTableName:@"aci_order_discount" forDataID:self.discountID];
        }
        
    }
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            [self requestUpdateRecordForTable:@"aci_order_discount" forDataID:self.discountID forKey:@"status" forValue:@"0" withTag:11];
        }
    }
...
-(void)datasocketDidReceiveNormalResponseWithDict:(NSDictionary *)resultDic andCustomerTag:(NSInteger)c_tag{
    if (c_tag == 1) {
        discount_pack = [CYFunctionSet stripNulls:[resultDic objectForKey:@"record"]];
        [self reloadContentTable];
    }
}
