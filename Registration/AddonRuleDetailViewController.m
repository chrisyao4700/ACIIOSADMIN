//
//  AddonRuleDetailViewController.m
//  ACI Hacienda
//
...
@interface AddonRuleDetailViewController ()

@end

@implementation AddonRuleDetailViewController{
...
-(IBAction)didClickRightItem:(id)sender{
    [self requestUpdateRecordForTable:@"aci_addon_rule" forDataID:self.addonRuleID forKey:@"status" forValue:@"1" withTag:11];
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self requestAddonRuleDetailData:self.addonRuleID];
    
}
-(void) requestAddonRuleDetailData:(NSString *) addonID{
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString * query = [NSString stringWithFormat:@"SELECT `aci_addon_rule`.*, CONCAT (`aci_contact`.`first_name`, ' ', `aci_contact`.`last_name`) AS `creator_name` FROM `aci_addon_rule` LEFT JOIN `aci_users` ON `aci_users`.`id` = `aci_addon_rule`.`cby` LEFT JOIN `aci_contact` ON `aci_contact`.`id` = `aci_users`.`contact_id` WHERE `aci_addon_rule`.`id` = %@",addonID];
        NSDictionary * dict = @{
                                @"query":query
                                };
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            //ID
            cell.textLabel.text = [NSString stringWithFormat:@"ADDON RULE # %@",self.addonRuleID];
        ...
        if (indexPath.row == 3) {
            ...
            cell.detailTextLabel.text = [rule_pack objectForKey:@"price"];
        }
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [self performAlertViewTextModifyWithKey:key_pool forValue:[rule_pack objectForKey:key_pool] forTableName:@"aci_addon_rule" forDataID:self.addonRuleID];
    }
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            [self requestUpdateRecordForTable:@"aci_addon_rule" forDataID:self.addonRuleID forKey:@"status" forValue:@"0" withTag:11];
        }
    }
}
